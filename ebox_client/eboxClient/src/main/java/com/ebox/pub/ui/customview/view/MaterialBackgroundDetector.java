...
public class MaterialBackgroundDetector {
    ...
    private static final int DEFAULT_ALPHA = 66;
    ...
    public void onSizeChanged(int width, int height) {
        ...
        if (DBG) {
            Log.i(TAG, "mWidth:" + mWidth+",mHeight:"+mHeight+" mViewRadius:"+mViewRadius);
        }
    }
    ...
    private void startShadowAnimation(MotionEvent event) {
        //When the fingers touch the view, we let the task appear slowly.
        mX = event.getX();
        mY = event.getY();
        ...
        mAnimator.start();
        if (DBG) {
            Log.i(TAG, "Down,from:" + 0 + ",to:" + mViewRadius+",mMinPadding "+mMinPadding);
        }
    }
    ...
    private void cancelShadowAnimation() {
        ...
        if (duration > 0) {
            //When the fingers leave the view, if the task doesn't cover whole view, we let the task appear fast.
    ...
    /**
     * We should let the task layer disappear gradually.
     */
    ...
    /**
     * This method is called by ObjectAnimator to let the task layer be transparent.
     *
     * @param alpha
     */
    ...
    public  int getColorAtAlpha(int color, int alpha) {
