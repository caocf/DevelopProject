...
public class Client
{
            ...
            HttpEntity entity, CallRet ret)
    {
        ...
        String contentType = header == null ? "application/octet-stream"
                : header.getValue();
    ...
    protected HttpResponse roundtrip(HttpRequestBase httpRequest)
            throws IOException
    {
        httpRequest.setHeader("Operators-Agent", Conf.getUserAgent());
            ...
            implements ICancel
    {
        ...
        protected Object doInBackground(Object... objects)
        {
            try
            {
                ...
                if (statusCode == 401)
                {
                    return new QiniuException(401, reqId, phrase); // android
                                                                   // 2.3 will
                                                                   // not
                                                                   // response
                }
                ...
                if (data.length > 0)
                {
                    return new QiniuException(statusCode, reqId, new String(
                            data));
                }
                if (xl.length() > 0)
                {
            ...
            {
                e.printStackTrace();
                return new QiniuException(QiniuException.IO, "net IOException",
                        e);
            }
        }
