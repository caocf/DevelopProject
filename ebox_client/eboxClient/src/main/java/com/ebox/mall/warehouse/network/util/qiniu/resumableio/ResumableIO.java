...
import com.ebox.mall.warehouse.network.util.qiniu.utils.FileUri;
...
public class ResumableIO
{
    ...
    private int putAndClose(final String key, final InputStreamAt input,
    ...
    public int put(final String key, final InputStreamAt input,
            final PutExtra extra, final JSONObjectRet ret)
    {
        ...
        for (int i = 0; i < blkCount; i++)
        {
            ...
            cancelers[i] = mClient.putblock(input, extra, extra.processes[i],
                    startPos, new JSONObjectRet(i)
                    {
                        int retryTime = 5;
                        ...
                        private void onAllSuccess()
                        {
                            ...
                            if (ctx.length() > 0)
                                ctx = ctx.substring(1);
                            removeTask(taskId);
                            mClient.mkfile(key, input.length(), extra.mimeType,
                                    extra.params, ctx, ret);
                        }

                        @Override
                        public void onSuccess(JSONObject obj)
                        {
                            if (failure[0] || ++success[0] != blkCount)
                                return;
                            onAllSuccess();
                        }
                        ...
                        public void onFailure(QiniuException ex)
                        {
                            ...
                            cancelers[mIdx] = mClient.putblock(input, extra,
                                    extra.processes[mIdx], startPos, this);
                        }
                    });
        }
    ...
    public int putFile(String key, File file, PutExtra extra,
            final JSONObjectRet ret)
    {
        return putAndClose(key, InputStreamAt.fromFile(file), extra, ret);
    }

    public int putFile(Context mContext, String key, Uri uri, PutExtra extra,
            final JSONObjectRet ret)
    {
    ...
    {
        return new ResumableIO(new ResumableClient(
                Client.getMultithreadClient(), uptoken));
    }

    public static int putAndClose(String uptoken, String key,
            InputStreamAt isa, PutExtra extra, JSONObjectRet ret)
    {
        return ResumableIO.defaultInstance(uptoken).putAndClose(uptoken, key,
                isa, extra, ret);
    }

    public static int put(String uptoken, String key, InputStreamAt isa,
            PutExtra extra, JSONObjectRet ret)
    {
        return ResumableIO.defaultInstance(uptoken).put(key, isa, extra, ret);
    }

    public static int putFile(Context mContext, String uptoken, String key,
            Uri uri, PutExtra extra, JSONObjectRet ret)
    {
        return ResumableIO.defaultInstance(uptoken).putFile(mContext, key, uri,
                extra, ret);
    }
