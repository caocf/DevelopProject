    ...
    public static void showNotification(Context context, String title, String content, String sound, Intent intent) {
        ...
        NotificationCompat.Builder mBuilder =
                new NotificationCompat.Builder(context)
                        ...
                        .setAutoCancel(true)
//                        .setSubText("T1111")设置二级内容
                        .setContentIntent(contentIntent)
                        .setDefaults(Notification.DEFAULT_VIBRATE | Notification.DEFAULT_SOUND)
                        .setContentText(content)
                        .setTicker(content);
        //获取通知的类型
        String tag = intent.getExtras().getString(Constants.NOTIFICATION_TAG);
        LogUtil.e("note ag:" + tag);
        if (!TextUtils.isEmpty(tag)) {
            if (tag.equals(Constants.NOTIFICATION_SINGLE_CHAT)) {//单聊显示用户名称
                mBuilder.setTicker(title + ":" + content);
            } else if (tag.equals(Constants.NOTIFICATION_SYSTEM)) {//系统通知
