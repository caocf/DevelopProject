...
import com.moge.gege.util.widget.horizontalListview.util.ViewHelperFactory;
import com.moge.gege.util.widget.horizontalListview.util.ViewHelperFactory.ViewHelper;
import com.moge.gege.util.widget.horizontalListview.util.v11.MultiChoiceModeListener;
import com.moge.gege.util.widget.horizontalListview.util.v11.MultiChoiceModeWrapper;
        ...
        ViewTreeObserver.OnTouchModeChangeListener
{
            ...
            case AccessibilityNodeInfoCompat.ACTION_SCROLL_FORWARD:
            {
                if (isEnabled() && getLastVisiblePosition() < getCount() - 1)
                {
                    final int viewportWidth = getWidth() - mListPadding.left
                            - mListPadding.right;
                    smoothScrollBy(viewportWidth,
                            PositionScroller.SCROLL_DURATION);
                    return true;
                }
            }
                return false;
                ...
                switch (mTouchMode)
                {
                    ...
                    case TOUCH_MODE_SCROLL:
                        ...
                        if (childCount > 0)
                        {
                            ...
                            final int lastChildRight = getChildAt(
                                    childCount - 1).getRight();
                            final int contentLeft = mListPadding.left;
                            final int contentRight = getWidth()
                                    - mListPadding.right;
                            if (mFirstPosition == 0
                                    && firstChildLeft >= contentLeft
                                    && mFirstPosition + childCount < mItemCount
                                    && lastChildRight <= getWidth()
                                            - contentRight)
                            {
                                mTouchMode = TOUCH_MODE_REST;
    ...
    private class FlingRunnable implements Runnable
    {
         ...
         */
        private final com.moge.gege.util.widget.horizontalListview.widget.OverScroller mScroller;
                ...
                case TOUCH_MODE_OVERFLING:
                {
                    ...
                    if (scroller.computeScrollOffset())
                    {
                        ...
                        if (overScrollBy(deltaX, 0, scrollX, 0, 0, 0,
                                mOverflingDistance, 0, false))
                        {
                            final boolean crossRight = scrollX <= 0
                                    && currX > 0;
        ...
        public void run()
        {
            ...
            switch (mMode)
            {
                case MOVE_DOWN_POS:
                {
                    ...
                    final int extraScroll = lastPos < mItemCount - 1 ? Math
                            .max(mListPadding.right, mExtraScroll)
                            : mListPadding.right;
                ...
                case MOVE_UP_POS:
                {
                    ...
                    final int extraScroll = firstPos > 0 ? Math.max(
                            mExtraScroll, mListPadding.left)
                            : mListPadding.left;
