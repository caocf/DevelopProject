...
package com.moge.gege.util.widget.horizontalListview.widget;
...
/**
 * A {@link android.widget.BaseAdapter} that provides data/Views in an
 * expandable list (offers features such as collapsing/expanding groups
 * containing children). By itself, this adapter has no data and is a connector
 ...
 * ListAdapter expects to/from group and child positions that the
 * ExpandableListAdapter expects.
 */
class ExpandableHListConnector extends BaseAdapter implements Filterable
{
    /**
     * The ExpandableListAdapter to fetch the data/Views for this expandable
     * list
     */
     ...
     * {@link ExpandableHListConnector.GroupMetadata}.
     */
    ...
    /**
     * Point to the {@link android.widget.ExpandableListAdapter} that will give
     * us data/Views
     *
     ...
     * corresponds to a child. Performs a binary search on the expanded groups
     * list to find the flat list pos if it is an exp group, otherwise finds
     * where the flat list pos fits in between the exp groups.
     *
     * @param flPos
     ...
     *         {@link ExpandableHListConnector.PositionMetadata} object that
     *         contains additional useful info for insertion, etc.
     */
    PositionMetadata getUnflattenedPos(final int flPos)
    {
        ...
        if (numExpGroups == 0)
        {
             ...
             * same as its flPos
             */
            return PositionMetadata.obtain(flPos,
                    ExpandableHListPosition.GROUP, flPos, -1, null, 0);
        }
         ...
         * the same as the flat list position (since there are no children
         * before it, and all groups before it are collapsed).
         */
        while (leftExpGroupIndex <= rightExpGroupIndex)
        {
            midExpGroupIndex = (rightExpGroupIndex - leftExpGroupIndex) / 2
                    + leftExpGroupIndex;
            ...
            else if (flPos == midExpGm.flPos)
            {
                 ...
                 */
                return PositionMetadata.obtain(flPos,
                        ExpandableHListPosition.GROUP, midExpGm.gPos, -1,
             ...
             */)
            {
                ...
                return PositionMetadata.obtain(flPos,
                        ExpandableHListPosition.CHILD, midExpGm.gPos, childPos,
                        midExpGm, midExpGroupIndex);
            }
        }
         ...
         * group that is not expanded, since otherwise we would have hit it in
         * the above search.
         */
        ...
        return PositionMetadata.obtain(flPos, ExpandableHListPosition.GROUP,
                groupPos, -1, null, insertPosition);
    }

    /**
     * Translates either a group pos or a child pos (+ group it belongs to) to a
     * flat list position. If searching for a child and its group is not
     * expanded, this will return null since the child isn't being shown in the
     * ListView, and hence it has no position.
     *
     * @param pos
     *            a {@link ExpandableHListPosition} representing either a group
     *            position or child position
     * @return the flat list position encompassed in a
     *         {@link ExpandableHListConnector.PositionMetadata} object that
     *         contains additional useful info for insertion, etc., or null.
     */
    PositionMetadata getFlattenedPos(final ExpandableHListPosition pos)
    {
        ...
        if (numExpGroups == 0)
        {
             ...
             * its flPos will be the same as its group pos. The insert position
             * is 0 (since the list is empty).
             */
            ...
            else if (pos.groupPos == midExpGm.gPos)
            {
                ...
                if (pos.type == ExpandableHListPosition.GROUP)
                {
                    ...
                    return PositionMetadata.obtain(midExpGm.flPos, pos.type,
                            pos.groupPos, pos.childPos, midExpGm,
                            midExpGroupIndex);
                }
                else if (pos.type == ExpandableHListPosition.CHILD)
                {
                    /* If it's a child, calculate the flat list pos */
                    return PositionMetadata.obtain(midExpGm.flPos
                            + pos.childPos + 1, pos.type, pos.groupPos,
    ...
    public boolean isEnabled(int flatListPos)
    {
        ...
        {
            retValue = mExpandableListAdapter.isChildSelectable(pos.groupPos,
                    pos.childPos);
        }
        else
        {
    ...
    public Object getItem(int flatListPos)
    {
        ...
        {
            retValue = mExpandableListAdapter.getChild(
    ...
    {
        final PositionMetadata posMetadata = getUnflattenedPos(flatListPos);
        final long groupId = mExpandableListAdapter
                .getGroupId(posMetadata.position.groupPos);
        ...
        {
            final long childId = mExpandableListAdapter.getChildId(
                    posMetadata.position.groupPos,
                    posMetadata.position.childPos);
            retValue = mExpandableListAdapter.getCombinedChildId(groupId,
                    childId);
        }
        else
        {
    ...
    public View getView(int flatListPos, View convertView, ViewGroup parent)
    {
        ...
        if (posMetadata.position.type == ExpandableHListPosition.GROUP)
        {
            retValue = mExpandableListAdapter.getGroupView(
                    posMetadata.position.groupPos, posMetadata.isExpanded(),
                    convertView, parent);
        }
        else if (posMetadata.position.type == ExpandableHListPosition.CHILD)
        {
            ...
            retValue = mExpandableListAdapter.getChildView(
                    posMetadata.position.groupPos,
                    posMetadata.position.childPos, isLastChild, convertView,
                    parent);
        }
        else
        {
    ...
    public int getItemViewType(int flatListPos)
    {
        ...
        if (mExpandableListAdapter instanceof HeterogeneousExpandableList)
        {
            ...
            {
                final int childType = adapter.getChildType(pos.groupPos,
                        pos.childPos);
     ...
     *            Forces refreshing of the children count for all expanded
     *            groups.
     * @param syncGroupPositions
     *            Whether to search for the group positions based on the group
     *            IDs. This should only be needed when calling this from an
     *            onChanged callback.
     */
            ...
            boolean syncGroupPositions)
    {
        ...
        for (int i = 0; i < egmlSize; i++)
        {
             ...
             */
            if ((curGm.lastChildFlPos == GroupMetadata.REFRESH)
                    || forceChildrenCountRefresh)
            {
                gChildrenCount = mExpandableListAdapter
                        .getChildrenCount(curGm.gPos);
            }
            else
            {
                /*
    ...
    boolean expandGroup(PositionMetadata posMetadata)
    {
        ...
        GroupMetadata expandedGm = GroupMetadata.obtain(GroupMetadata.REFRESH,
                GroupMetadata.REFRESH, posMetadata.position.groupPos,
     ...
     *         {@link android.widget.AdapterView#INVALID_POSITION} if it can't
     *         be found
    ...
    static class GroupMetadata implements Parcelable, Comparable<GroupMetadata>
    {
        ...
        static GroupMetadata obtain(int flPos, int lastChildFlPos, int gPos,
                long gId)
        {
    ...
    static public class PositionMetadata
    {
        ...
        /**
         * Use
         * {@link #obtain(int, int, int, int, ExpandableHListConnector.GroupMetadata, int)}
         */
                ...
                int childPos, GroupMetadata groupMetadata, int groupInsertIndex)
        {
            ...
            pm.position = ExpandableHListPosition.obtain(type, groupPos,
                    childPos, flatListPos);
